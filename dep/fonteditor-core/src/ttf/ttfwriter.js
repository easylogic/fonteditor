...
define(
    function (require) {
        ...
        function resolve(ttf) {
			...
			if (ttf.COLR ) {
				ttf.writeOptions.tables.push('COLR');
			}
			if (ttf.CPAL ) {
				ttf.writeOptions.tables.push('CPAL');
			}
			if (ttf.SVG ) {
				ttf.writeOptions.tables.push('SVG');
			}

			if (ttf.CFF ) {
				ttf.version = 0x4F54544F;  // OTTO
				ttf.writeOptions.tables.push('CFF');
				ttf.isCFF = true; 
			}

			ttf.writeOptions.tables.sort();
			ttf.numTables = ttf.writeOptions.tables.length;
        ...
        function write(ttf) {
			...
			if (ttf.isCFF)
			{
				writer.writeUint32(ttf.version);
			} else {
