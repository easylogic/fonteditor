...
define(
    function (require) {
		 ...
		 *
		 */
        TTF.prototype.insertMultipleGlyf = function (glyfList, isFilteredForName) {
				
			if (isFilteredForName)
			{
				var keys = this.ttf.glyf.map(function(g) { return g.name;  });

				keys = keys.sort(function(a, b) { return a - b }).reduce(function(a, b) {
					if (a.slice(-1)[0] !== b)
					{
						a.push(b);
					}
				...
				// name  이 없는 애들만 추가 
				glyfList  = glyfList.filter(function(g) {
					return keys.indexOf(g.name) == -1;	
				});	
			}

            this.ttf.glyf.push.apply(this.ttf.glyf, glyfList);
		...
		TTF.prototype.getMapIndex = function () {
			var obj = {};

			for(var i = 0, len = this.ttf.glyf.length; i < len; i++) {
				var g = this.ttf.glyf[i];

				if (g.unicode)
				{
					g.unicode.forEach(function(unicode) {
						obj[unicode] = i;
					});
				}
        ...
        TTF.prototype.sortGlyf = function () {
            ...
            if (glyf.length > 1) {
				...
				// notdef 가 어디든 있을 수 있으므로 unicode 가 있는 것부터 차례로 비교한다. 
